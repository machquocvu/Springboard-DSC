...
SELECT name, monthlymaintenance,
	CASE WHEN monthlymaintenance >100 THEN 'expenisve' 
...
FROM (
	...
	ON b.memid = m.memid
	WHERE f.name LIKE '%TENNIS COURT%') AS sub
...
WITH sub AS(
    SELECT f.name, b.memid, b.starttime,
		CASE WHEN b.memid = 0 THEN (b.slots * f.guestcost * 2) ELSE (b.slots * f.membercost * 2) END AS cost
	FROM Bookings AS b
	LEFT JOIN Facilities AS f
	ON b.facid = f.facid)
SELECT s.name AS court, CONCAT(m.firstname, ' ', m.surname) AS member, s.cost, s.starttime
FROM sub AS s
LEFT JOIN Members AS m
ON s.memid = m.memid
WHERE s.cost > 30 AND DATE(s.starttime) = '2012-09-14'
ORDER BY s.cost DESC;


/* Q9: This time, produce the same result as in Q8, but using a subquery. */

SELECT sub.name AS court, CONCAT(m.firstname, ' ', m.surname) AS member, sub.cost, sub.starttime
FROM(
	SELECT f.name, b.memid, b.starttime,
		CASE WHEN b.memid = 0 THEN (b.slots * f.guestcost * 2) ELSE (b.slots * f.membercost * 2) END AS cost
	FROM Bookings AS b
...
WHERE sub.cost > 30 AND DATE(sub.starttime) = '2012-09-14'
